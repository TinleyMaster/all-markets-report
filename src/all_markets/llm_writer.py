...
def polish_report_with_deepseek(bundle: ReportBundle, api_key: str, model: str) -> ReportBundle:
...
要求：
1. 保留原始结论方向，不得新增未提供的市场或数字。
2. 语言简洁、有信息密度，避免空话。
3. 输出必须是 Markdown。
4. 维持以下结构：标题、今日一句话、四分法结论、资金流向地图、为什么其他市场跌、宏观因子、全球基准表现、主题表现。
5. 每个“为什么其他市场跌”都要带一句逻辑解释。
