...
def _leader_reason(snapshot: SymbolSnapshot, macro_context: dict[str, SymbolSnapshot]) -> str:
    reasons: list[str] = []
    ...
    if snapshot.daily_return > 1.5:
        reasons.append("日内有明显增量资金追价")

    if snapshot.group in {"us_ai", "us_tech", "us_semiconductor", "korea_semiconductor", "taiwan_semiconductor"}:
        if macro_context.get("duration") and macro_context["duration"].daily_return > 0:
            reasons.append("利率压力边际缓和有利成长板块")
        else:
            reasons.append("AI与半导体仍是资金主线")
    elif snapshot.group in {"energy"}:
        reasons.append("油价偏强支撑能源股")
...
def _macro_flags(macro_context: dict[str, SymbolSnapshot]) -> list[str]:
    ...
    if volatility and volatility.daily_return > 5:
        flags.append("VIX抬升，风险偏好回落")
    elif volatility and volatility.daily_return < -5:
        flags.append("VIX回落，风险偏好修复")
...
def _build_regime(macro_context: dict[str, SymbolSnapshot], top_themes: list[MarketLeader]) -> tuple[str, str, str]:
    dollar = macro_context.get("dollar")
    duration = macro_context.get("duration")
    volatility = macro_context.get("volatility")

    growth_bias = any(item.symbol in {"QQQ", "XLK", "SOXX", "SMH", "NVDA", "MSFT", "005930.KS", "000660.KS"} for item in top_themes)
    risk_on = (
        (volatility and volatility.daily_return < 0)
        and (dollar and dollar.daily_return <= 0.3)
        and growth_bias
    )

    if risk_on:
        regime = "风险偏好回升"
        narrative = "全球资金正在从防御与低弹性资产回流成长，主线集中在AI与半导体。"
        positioning = "仓位判断：偏进攻，但继续围绕盈利确定性更强的科技龙头。"
    elif duration and duration.daily_return < -0.5:
        regime = "高利率压制"
        narrative = "市场重新交易利率上行，成长与新兴市场承压，资金更偏向防御与现金流资产。"
        positioning = "仓位判断：以均衡偏防守为主，回避高估值弱基本面资产。"
    else:
        regime = "震荡轮动"
        narrative = "全球市场缺乏单一主线，资金更偏向区域与行业间的相对收益切换。"
